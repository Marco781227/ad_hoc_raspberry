...
server.bind((HOST, PORT)) # Liaison du socket à l'adresse et au port spécifiés (HOST et PORT)
server.listen(5)
...
conn, addr = server.accept() # Attente de la connexion d'un client et récupération de la connexion et de l'adresse du client
...
while(True):
	...
	print(len(data))
	timestamp, latitude, longitude = json.loads(data.decode()) # Décodage des données reçues depuis bytes vers string, puis conversion du JSON en une liste ou tuple
